"use strict";

var _steps = require("cypress-cucumber-preprocessor/steps");

(0, _steps.Given)('a Cover with a button below it is rendered', () => {
  cy.visitStory('Cover', 'Blocking');
});
(0, _steps.Given)('a Cover with a button in it is rendered', () => {
  cy.visitStory('Cover', 'With Click Handler');
});
(0, _steps.When)('the user clicks the button', () => {
  cy.get('button').click();
});
(0, _steps.When)('the user clicks on the Cover', () => {
  cy.get('[data-test="dhis2-uicore-componentcover"]').click();
});
(0, _steps.When)('the user clicks on the button coordinates', () => {
  cy.getPositionsBySelectors('button').then(_ref => {
    let [rect] = _ref;
    // Get button center coordinates
    const buttonCenterX = rect.left + rect.width / 2;
    const buttonCenterY = rect.top + rect.height / 2; // click body on the button center

    cy.get('body').click(buttonCenterX, buttonCenterY);
  });
});
(0, _steps.Then)('the onClick handler of the button is called', () => {
  cy.window().should(win => {
    expect(win.onButtonClick).to.be.calledOnce;
  });
});
(0, _steps.Then)('the onClick handler of the Cover is called', () => {
  cy.window().should(win => {
    expect(win.onCover).to.be.calledOnce;
  });
});
(0, _steps.Then)('the onClick handler of the button is not called', () => {
  cy.window().should(win => {
    expect(win.onButtonClick).to.have.callCount(0);
  });
});
(0, _steps.Then)('the onClick handler of the Cover is not called', () => {
  cy.window().should(win => {
    expect(win.onCover).to.have.callCount(0);
  });
});