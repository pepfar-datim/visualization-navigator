 ...
 */
// Test 1
(0, _steps.Given)('a reference element in a scrolling container with enough space between the top of the reference element and the container top to fit the Tooltip, and there is enough space above the container for a tooltip', () => {
  cy.visitStory('Tooltip', 'Scrolling Containers');
});
(0, _steps.When)('the mouse cursor enters the anchor without scrolling', () => {
  cy.get('[data-test="dhis2-uicore-tooltip-reference"]').trigger('mouseover', 'top', {
    scrollBehavior: false
  });
}); // Test 2

(0, _steps.When)('the container is scrolled such that the reference element is still visible but tooltip in its default position would leave the container boundaries', () => {
  cy.get('[data-test="hiding-and-flipping-container"]').scrollTo(0, 250);
});
(0, _steps.When)('the mouse cursor enters the bottom of the anchor without scrolling', () => {
  cy.get('[data-test="dhis2-uicore-tooltip-reference"]').trigger('mouseover', 'bottom', {
    scrollBehavior: false
  });
}); // Test 3

(0, _steps.Given)('this test uses timers', () => {
  cy.clock();
});
(0, _steps.And)('there is a pause to allow the tooltip to open', () => {
  cy.tick(250);
});
(0, _steps.When)('the container is scrolled such that the reference element is not visible', () => {
  cy.get('[data-test="hiding-and-flipping-container"]').scrollTo(0, 320);
});
(0, _steps.When)('the mouse cursor leaves the anchor without scrolling', () => {
...
(0, _steps.Then)('the Tooltip still exists but is hidden', () => {
...
(0, _steps.When)('enough time passes for the CLOSE_DELAY to complete', () => {
  cy.clock();
  cy.tick(500);
});