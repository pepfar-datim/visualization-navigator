...
(0, _steps.When)('the user clicks multiple items with {string} which are not highlighted', modifierKey => {
  cy.get('@list').find('{transferoption}').then($options => {
    const multipleOptions = $options.filter(index => index < 3);
    return cy.wrap(multipleOptions);
  }).as('multipleOptions').each($option => cy.wrap($option).clickWith(modifierKey === 'cmd' ? 'meta' : modifierKey));
});
(0, _steps.When)('the user clicks on one item with {string} which is highlighted', modifierKey => {
...
(0, _steps.When)('the user clicks on one item without a modifier key which is highlighted', () => {
  cy.get('@highlightedMultipleOptions').first().click().as('clickedOption');
  cy.get('@highlightedMultipleOptions').filter(index => index !== 0).as('remainingHighlightedOptions');
});
(0, _steps.Then)('all of the clicked items should be highlighted', () => {
  cy.get('@multipleOptions').each($option => {
    expect($option).to.have.class('highlighted');
  });
});
(0, _steps.Then)('the clicked item should not be highlighted', () => {
  cy.get('@clickedOption').should('not.have.class', 'highlighted');
});
(0, _steps.Then)('the other previously highlighted items should remain highlighted', () => {
  cy.get('@remainingHighlightedOptions').each($option => {
    expect($option).to.have.class('highlighted');
  });
});
(0, _steps.Then)('the clicked option is highlighted', () => {
  cy.get('@clickedOption').should('have.class', 'highlighted');
});
(0, _steps.Then)('the other previously highlighted items should not be highlighted', () => {
  cy.get('@remainingHighlightedOptions').each($option => {
