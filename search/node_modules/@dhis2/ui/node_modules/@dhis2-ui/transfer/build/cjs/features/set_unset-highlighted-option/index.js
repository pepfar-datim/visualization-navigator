...
(0, _steps.Given)('the highlighted item is not visible due to a set filter', () => {
...
(0, _steps.When)('the user clicks an item in the list that is not highlighted', () => {
  cy.get('@list').find('{transferoption}').first().invoke('next').as('nextHighlighted').click();
});
(0, _steps.When)('the user clicks an item in the list that is highlighted', () => {
  cy.get('@initiallyHighlighted').wait(500).click();
});
(0, _steps.When)('the user selects the visible, highlighted options', () => {
  cy.get('{transfer-actions-addindividual}').click();
});
(0, _steps.When)('the users changes the filter to include the hidden option', () => {
  cy.get('{transfer-filter} input').clear();
});
(0, _steps.Then)('the clicked item should be highlighted', () => {
  cy.get('@nextHighlighted').should('have.class', 'highlighted');
});
(0, _steps.Then)('the previously highlighted item should no longer be highlighted', () => {
