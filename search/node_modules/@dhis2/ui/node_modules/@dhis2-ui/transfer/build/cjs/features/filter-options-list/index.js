...
(0, _steps.Given)('filtering is enabled', () => {// no op
});
(0, _steps.Given)('the options list is being filtered', () => {// no op
});
(0, _steps.Given)('the result is not empty', () => {
  cy.visitStory('Transfer filtering', 'Some Results');
});
(0, _steps.Given)('the result is empty', () => {
  cy.visitStory('Transfer filtering', 'Empty Result');
});
(0, _steps.Given)('a no-result message has been provided', () => {// no op
});
(0, _steps.Given)('the options are being search with a {string} search term', firstCase => {
...
(0, _steps.Given)('some options are listed', () => {
  cy.get('{transfer-sourceoptions} {transferoption}').should('have.length.of.at.least', 1).as('firstCaseOptions');
});
(0, _steps.Given)('the filter function only returns ANC options', () => {
  cy.visitStory('Transfer filtering', 'Anc Custom Filter');
});
(0, _steps.Given)("the filter value is controlled by the component's consumer", () => {
...
(0, _steps.When)('searching for "s"', () => {
  cy.get('{transfer-filter} input').type('s');
});
(0, _steps.When)('the filter value changes', () => {
...
(0, _steps.Then)('no items should be shown in the options list', () => {
  cy.get('{transferoption}').should('not.exist');
});
(0, _steps.Then)('information should be displayed that no items matched the filter', () => {
  cy.get('{no-results}', {
    prefix: ''
  }).should('exist');
});
...
(0, _steps.Then)('only the results including the word "ANC" are included', () => {
  cy.get('{transfer-sourceoptions} {transferoption}').each($option => expect($option.text()).to.match(/ANC/));
});
(0, _steps.Then)('the onFilterChange callback will be called with the new value', () => {
